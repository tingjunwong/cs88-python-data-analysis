...
CREATE TABLE long_album as
  select a.title from albums as a, tracks as b where a.albumId = b.albumId and b.milliseconds > 360000;
...
CREATE TABLE album_count as
  select a.name, count(b.artistID) from artists as a, albums as b where a.artistID = b.artistID group by a.name;
-- select name, count(artistId) from artists group by name;

-- Question 8
CREATE TABLE busiest_artists as
  select a.name, count(c.albumID) from artists as a, albums as b, tracks as c where a.artistID = b.artistID and b.albumId = c.albumId group by a.name;
